namespace BayuOrtak.Core.Wcf.Nhr.Helper
{
    ...
    /// <summary>
    /// NHRTools sınıfı, personel iş kayıtları ve özelliklerine dair çeşitli yardımcı metotlar sağlar. Bu sınıf, personel unvanları, durum tipleri ve özel durum kontrolleri gibi işlevleri yerine getirir.
    /// </summary>
    public sealed class NHRTools
    {
        /// <summary>
        /// Belirtilen T.C. Kimlik Numarası için Personel Otomasyon Sisteminde (NHR) kayıt bulunamadığında hata mesajı döner. Kullanıcının tercih ettiği dile göre mesaj oluşturur (Türkçe veya İngilizce).
        /// </summary>
        ...
        public static string TcknNotFound(long tckn, string dil)
        {
            ...
            return $"Personel Otomasyonunda(NHR) \"{tckn}\" T.C. Kimlik Numarası üzerine kayıt bulunamadı. Personel Daire Başkanlığı veya Bilgi İşlem Daire Başkanlığı ile iletişime geçiniz!";
        }
        /// <summary>
        ...
        {
            var _r = 0;
            if (IsEngelliPersonel(personelozurorani, personelozurdurumu)) { _r = _r + (int)Nhr_DurumTypes.engelli; }
